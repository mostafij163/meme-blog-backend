...
export class UserService {
    ...
    async createUser(data: SignUpDTO) {
        if (data.password !== data.confirmPassword) throw new UnprocessableEntityException("Password did not matched")
        try {
            ...
            return this.jwtService.signToken({email: user.email}, {subject: `${user["_id"]}`})
        } catch (err) {
            console.log(err)
